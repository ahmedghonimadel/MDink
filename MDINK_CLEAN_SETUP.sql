-- ============================================================
-- MDink Solutions — MDINK_CLEAN_SETUP.sql (إعداد نظيف + حماية Super Admin)
-- ============================================================
-- شغّلي هذا الملف مرة واحدة بالكامل في Supabase → SQL Editor → Run.
-- خُذي Backup أولًا (Database → Backups) قبل التشغيل.
--
-- ماذا يفعل هذا الملف بالضبط:
-- ------------------------------------------------------------
-- 1) DROP (حذف وإعادة بناء) للجداول التالية فقط — وهي جداول
--    "محتوى الموقع/CMS" التي يُعاد زرع محتواها بالكامل داخل هذا
--    الملف نفسه، لذا لا يضيع أي محتوى (النسخة القديمة قد تكون
--    بأعمدة مختلفة تسبب أخطاء column/ON CONFLICT):
--       site_config, dynamic_links, contact_channels,
--       cms_pages, cms_services, portfolio_items, blogs,
--       seo_settings, client_testimonials, testimonial_submissions,
--       testimonials, services, about_gallery
--    كل محتوى هذه الجداول (الصفحات، الخدمات، الأعمال، المقالات،
--    الشهادات، إعدادات SEO...) يُعاد إدخاله داخل هذا الملف، فالنتيجة
--    هي أحدث نسخة نظيفة من محتوى الموقع.
--
-- 2) لا يُحذف ولا يُلمس إطلاقًا (يُحافَظ عليها ويُضاف لها الأعمدة
--    الناقصة فقط عبر ADD COLUMN IF NOT EXISTS):
--       user_roles, profiles, admin_allowlist  ← (ربط المستخدمين والصلاحيات)
--       leads, consultations, doctor_applications, clients/*payments,
--       team_profiles, team_members, notifications, daily_work_logs,
--       client_financials, permissions, user_permissions, ...
--    أي بيانات مستخدمين/طلبات/عملاء موجودة تبقى كما هي.
--
-- 3) لا يمس منطق لوحة إدارة الموقع (الكود) — فقط يضبط الجداول
--    التي يقرأ/يكتب منها. محرّرات الـCMS في الداشبورد تظل تعمل.
--
-- 4) حماية Super Admin (في نهاية الملف):
--    الإيميلان shfahmy2010@gmail.com و tasneemfahmy21@gmail.com
--    يظلّان super_admin أساسيين، ولهما كل الصلاحيات، ومحميّان ضد
--    حذف/سحب دور super_admin من جداول الأدوار (عبر TRIGGER + دالة).
--    الماليات (client_financials) مقروءة لهما فقط عبر RLS.
--    (ملاحظة: حذف الحساب نفسه من Supabase Auth يتم من لوحة Supabase
--     وهو خارج تحكّم قاعدة البيانات، لكن الأدوار/الصلاحيات محميّة بالكامل.)
--
-- 5) الـ Edge Function باسم create-admin-user لا يمسّها هذا الملف
--    إطلاقًا — تبقى كما هي في مشروعك.
-- ============================================================

-- ── (أ) حذف جداول محتوى الموقع المتعارضة فقط، لإعادة بنائها نظيفة ──
-- (آمن: محتواها يُعاد زرعه بالكامل أدناه. CASCADE لإزالة أي اعتماديات قديمة.)
...
-- ملاحظة: user_roles / profiles / admin_allowlist لا تُحذف إطلاقًا (حماية الصلاحيات).
...
DO $$ BEGIN
  ALTER TABLE public.user_roles ADD CONSTRAINT user_roles_user_id_role_uq UNIQUE (user_id, role);
EXCEPTION
...
DO $$ BEGIN
  ALTER TABLE public.profiles ADD CONSTRAINT profiles_id_uq UNIQUE (id);
EXCEPTION
  WHEN duplicate_table THEN NULL;
  WHEN duplicate_object THEN NULL;
  WHEN others THEN NULL;
...
DO $$ BEGIN
  ALTER TABLE public.site_config ADD CONSTRAINT site_config_key_uq UNIQUE (key);
EXCEPTION
  WHEN duplicate_table THEN NULL;
  WHEN duplicate_object THEN NULL;
  WHEN others THEN NULL;
...
DO $$ BEGIN
  ALTER TABLE public.blogs ADD CONSTRAINT blogs_slug_uq UNIQUE (slug);
EXCEPTION
  WHEN duplicate_table THEN NULL;
  WHEN duplicate_object THEN NULL;
  WHEN others THEN NULL;
...
DO $$ BEGIN
  ALTER TABLE public.admin_allowlist ADD CONSTRAINT admin_allowlist_email_uq UNIQUE (email);
EXCEPTION
  WHEN duplicate_table THEN NULL;
  WHEN duplicate_object THEN NULL;
  WHEN others THEN NULL;
...
DO $$ BEGIN
  ALTER TABLE public.portfolio_items ADD CONSTRAINT portfolio_items_slug_uq UNIQUE (slug);
EXCEPTION
  WHEN duplicate_table THEN NULL;
  WHEN duplicate_object THEN NULL;
  WHEN others THEN NULL;
...
DO $$ BEGIN
  ALTER TABLE public.cms_pages ADD CONSTRAINT cms_pages_key_uq UNIQUE (key);
EXCEPTION
  WHEN duplicate_table THEN NULL;
  WHEN duplicate_object THEN NULL;
  WHEN others THEN NULL;
...
-- (تم حذف جمل ترقية قديمة كانت تنقل من أعمدة legacy — غير لازمة في الإعداد النظيف)
...
DO $$ BEGIN
  ALTER TABLE public.seo_settings ADD CONSTRAINT seo_settings_page_key_uq UNIQUE (page_key);
EXCEPTION
  WHEN duplicate_table THEN NULL;
  WHEN duplicate_object THEN NULL;
  WHEN others THEN NULL;
...
-- إزالة قيود NOT NULL عن أعمدة قديمة قد تكون إجبارية في الجداول السابقة
DO $$ BEGIN
  BEGIN ALTER TABLE public.client_testimonials ALTER COLUMN quote_ar DROP NOT NULL; EXCEPTION WHEN others THEN NULL; END;
  BEGIN ALTER TABLE public.client_testimonials ALTER COLUMN quote_en DROP NOT NULL; EXCEPTION WHEN others THEN NULL; END;
  BEGIN ALTER TABLE public.client_testimonials ALTER COLUMN avatar_url DROP NOT NULL; EXCEPTION WHEN others THEN NULL; END;
END $$;
...
DO $$ BEGIN
  ALTER TABLE public.permissions ADD CONSTRAINT permissions_permission_key_uq UNIQUE (permission_key);
EXCEPTION
  WHEN duplicate_table THEN NULL;
  WHEN duplicate_object THEN NULL;
  WHEN others THEN NULL;
...
-- ============================================================
-- حماية Super Admin — الإيميلان الأساسيان
--   shfahmy2010@gmail.com  /  tasneemfahmy21@gmail.com
-- ============================================================

-- (1) تأكيد أن الإيميلين مُدرَجان في admin_allowlist (طبقة حماية إضافية)
--     admin_allowlist يُستخدم في is_admin() كـ fallback.
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables
             WHERE table_schema='public' AND table_name='admin_allowlist') THEN
    -- أضف عمود email لو ناقص (حماية ضد الأشكال القديمة)
    BEGIN ALTER TABLE public.admin_allowlist ADD COLUMN IF NOT EXISTS email text; EXCEPTION WHEN others THEN NULL; END;
    INSERT INTO public.admin_allowlist (email)
    SELECT e FROM (VALUES ('shfahmy2010@gmail.com'), ('tasneemfahmy21@gmail.com')) v(e)
    WHERE NOT EXISTS (
      SELECT 1 FROM public.admin_allowlist a WHERE lower(a.email) = lower(v.e)
    );
  END IF;
END $$;

-- (2) امنح الإيميلين دور super_admin في user_roles إن كان لهما حساب Auth
--     (يعمل الآن لو كانا مسجّلين، ويُعاد ضمانه عبر التريجر لاحقًا)
INSERT INTO public.user_roles (user_id, role)
...
WHERE lower(u.email) IN ('shfahmy2010@gmail.com','tasneemfahmy21@gmail.com')
  AND NOT EXISTS (
    ...
    WHERE r.user_id = u.id AND r.role::text = 'super_admin'
  );

-- (3) دالة الحماية: تمنع حذف أو تعديل دور super_admin للإيميلين
...
DECLARE
  target_email text;
BEGIN
  -- عند الحذف: امنع حذف صف super_admin الخاص بالإيميلين
  IF (TG_OP = 'DELETE') THEN
    SELECT lower(u.email) INTO target_email FROM auth.users u WHERE u.id = OLD.user_id;
    IF OLD.role::text = 'super_admin'
       AND target_email IN ('shfahmy2010@gmail.com','tasneemfahmy21@gmail.com') THEN
      RAISE EXCEPTION 'لا يمكن سحب دور super_admin من حساب أساسي محمي (%).', target_email;
    END IF;
    RETURN OLD;
  END IF;

  -- عند التعديل: امنع تغيير دور super_admin الخاص بالإيميلين إلى أي دور آخر
  IF (TG_OP = 'UPDATE') THEN
    SELECT lower(u.email) INTO target_email FROM auth.users u WHERE u.id = OLD.user_id;
    IF OLD.role::text = 'super_admin'
       AND NEW.role::text <> 'super_admin'
       AND target_email IN ('shfahmy2010@gmail.com','tasneemfahmy21@gmail.com') THEN
      RAISE EXCEPTION 'لا يمكن تغيير دور super_admin لحساب أساسي محمي (%).', target_email;
    END IF;
    RETURN NEW;
  END IF;
...
DROP TRIGGER IF EXISTS trg_protect_super_admins ON public.user_roles;
CREATE TRIGGER trg_protect_super_admins
  BEFORE UPDATE OR DELETE ON public.user_roles
  FOR EACH ROW EXECUTE FUNCTION public.protect_super_admins();

-- (4) دالة يدوية لإعادة ضمان صلاحية الإيميلين وقت الحاجة (بعد إنشاء حساباتهم)
CREATE OR REPLACE FUNCTION public.ensure_super_admins()
RETURNS void
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
...
-- تشغيلها مرة الآن (وتُشغَّل يدويًا لاحقًا لو أُنشئ الحسابان بعد هذا الملف):
SELECT public.ensure_super_admins();
