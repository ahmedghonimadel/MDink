...
export function useSiteConfig() {
  const { data } = useQuery({
    ...
    queryFn: async (): Promise<SiteConfigMap> => {
      ...
      if (s) {
        if (s.brand_name) map.site_name = s.brand_name;
        // اسم الموقع بالإنجليزي: عمود مستقل إن وُجد، وإلا نستعمل العربي كخيار احتياطي
        map.site_name_en = s.brand_name_en || s.brand_name || map.site_name_en;
        ...
        // نص الفوتر (عربي/إنجليزي) — يعتمد على الأعمدة الجديدة
        if (s.footer_about_text) map.footer_about_text = s.footer_about_text;
        if (s.footer_about_text_en) map.footer_about_text_en = s.footer_about_text_en;
      }
